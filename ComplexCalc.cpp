...
// the constant PI
double re1,re2,im1,im2,re3,im3;
//(re1,im1)first num(re2,im2)second num(re3,im3)result. int count;
// the counter of your choise (add,subtr,...).
...
return 0; }
int complex()
cout<<setw(47)<<"Complex Calc Project\n"<<endl; cout<<setw(58)<<"04MAY2020"<<endl<<endl<<endl; complex();
 {
cout<<"\n"<<setw(57)<<" *COMPLEX NUMBERS*
\n\n"<<setw(61)<<"######################################\n\n\n\n"; do
{ number1:
cout<<"\tEnter the first complex number Z1 in the format x+iy: "; cin>>re1>>sign1>>i1>>im1;
         if(sign1!='+')
        {
            if(sign1!='-')
            {
...
number2:
 cout<<"\n\n\tEnter the second complex number Z2 in the format x+iy: "; cin>>re2>>sign2>>i2>>im2;
if(sign2!='+')
{
    if(sign2!='-')
...
break;
case 3://Multiplication cout<<endl<<setw(48)<<"\"Multiplication\"\n\n"; multiply();
break;
case 4://Division cout<<endl<<setw(45)<<"\"Division\"\n\n"; divide();
break;
case 5://Conjugate cout<<endl<<setw(45)<<"\"Conjugate\"\n\n";
     conjugate();
break;
case 6://Angle(theta)
...
case 8://Transform to trigenometric form
 form\"\n\n";
cout<<endl<<setw(56)<<"\"Transform to trigenometric
    convert_tri();
break;
case 9://Transform to polar form
cout<<endl<<setw(52)<<"\"Transform to polar form\"\n\n"; convert_pol();
break;
case 10://Transform to Exponential form cout<<endl<<setw(55)<<"\"Transform to Exponential
    convert_expon();
break;
...
cout<<"\n\nDo you want to enter anew numbers?(y/n)"; cin>>choise; //to enter anew numbers or not.
         clearScreen();
        cout<<endl<<endl;
} while(choise=='y'||choise=='Y'); return (0);
}
int add()
{
double q,p;
...
if(sign1=='+' && sign2=='-' ) {
    ...
    im3=q+p;
    if(im3<0)
cout<<"\n\tZ1 + Z2 = "<<re3<<" - i "<<im3*(-1)<<endl; else
cout<<"\n\tZ1 + Z2 = "<<re3<<" + i "<<im3<<endl; return 0;
}
int subtract()
{
double q,p;
 ...
 if(sign1=='-')
{
    ...
    if(sign2=='+')
    {
        re3=(re1*re2)-(im1*im2);
im3=(-re1*im2)+(-re2*im1); }
} deno=(re2*re2)+(im2*im2); if(im3<0)
 cout<<"\n\tZ1 / Z2 = "<<re3<<"/"<<deno<<" - i "<<im3*(- 1)<<"/"<<deno<<endl;
else
cout<<"\n\tZ1 / Z2 = "<<re3<<"/"<<deno<<" + i "<<im3<<"/"<<deno<<endl; return 0;
}
...
float g,h;
cout<<"\n\tThe angle of the first number \xE9"<<"1 = "; g=atan(q/re1);
g=(g*180)/pi;
if(g<0)
g*=-1;
 if(re1>0&&q>0)
...
if(re1<0&&q<0)
 ...
 if(re1>=0&&q<0)
    g=360-g;
cout<<g<<" grade"<<endl;
cout<<"\n\tThe angle of the second number \xE9"<<"2 = "; h=atan(p/re2);
h=(h*180)/pi;
     if(h<0)
        h*=-1;
    if(re2>0&&p>0)
...
cout<<"\n\tThe modulus of the first number (r1)="; r1=((re1*re1)+(q*q));
cout<<" root "<<(r1)<<endl;
cout<<"\n\tThe modulus of the second number (r2)="; r2=((re2*re2)+(p*p));
 cout<<" root "<<(r2)<<endl; return 0;
}
int convert_tri()
{
float g,h;
